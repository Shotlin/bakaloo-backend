...
export class ScratchCardRepository {
  ...
  async getSettings() {
    const { rows } = await query(
      `SELECT id, daily_free_scratches, trigger_mode,
  ...
  async updateSettings(data) {
    ...
    const { rows } = await query(
      `UPDATE scratch_card_settings SET ${fields.join(', ')}
       RETURNING id, daily_free_scratches, trigger_mode,
                 cover_image_url, cover_image_public_id, updated_at`,
      params
    )
    return rows[0] ? this._formatSettings(rows[0]) : null
  ...
  async insertHistory(client, { userId, prizeId, prizeType, prizeLabel, prizeValue, isWin, rewardStatus }) {
    const runner = client ? client.query.bind(client) : query
    const { rows } = await runner(
      `INSERT INTO scratch_history (user_id, prize_id, prize_type, prize_label, prize_value, is_win, reward_status)
  ...
  async listHistory({ limit = 20, offset = 0, userId = null } = {}) {
    const { rows } = await query(
      `SELECT h.id, h.user_id, u.name AS user_name, u.phone AS user_phone,
              h.prize_type, h.prize_label, h.prize_value, h.is_win, h.reward_status, h.reward_ref, h.scratched_at
