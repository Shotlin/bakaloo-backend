...
-- 'B2B_CREDIT') distinct from the existing exclusive LEDGER method: it's
-- available to any customer with an APPROVED, b2b_enabled business account
-- (see BusinessAccountsRepository), with NO credit limit or cap of any
-- kind — a B2B customer can place an order for any amount. Unlike every
...
--      sanity check.
--   3. Settlement (how the customer actually paid — cash, UPI, Razorpay,
--      or another method) is recorded manually by an admin after delivery,
--      potentially across multiple partial entries over time
--      (order_b2b_settlements below). There is no ledger/credit-limit
--      balance behind this — "amount owed" is purely
--      orders.total_amount - orders.b2b_amount_settled, per order.
--   4. The customer can optionally be given a payment due date up front
--      (e.g. "I'll pay in 5 days") — see b2b_payment_due_date below.
...
-- denormalized so the B2B Orders/Collections lists can show "settled ₹X of
-- ₹Y" without a join+SUM per row. Updated transactionally alongside each
-- settlement insert (see AdminOrdersService#recordB2BSettlement).
ALTER TABLE orders
  ADD COLUMN IF NOT EXISTS b2b_amount_settled DECIMAL(10,2) NOT NULL DEFAULT 0;
-- Optional admin-set date the customer promised to pay by (e.g. "5 days
-- later I will pay") — purely informational/schedule-tracking, nothing
-- automated hangs off it (no auto-charge, no auto-suspension).
ALTER TABLE orders
...
-- against a "Place Order" order after delivery (e.g. ₹200 cash + ₹500 UPI
-- for a ₹700 order, entered as two separate rows over time).
CREATE TABLE IF NOT EXISTS order_b2b_settlements (
  id           UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  order_id     UUID NOT NULL REFERENCES orders(id) ON DELETE CASCADE,
  method       VARCHAR(20) NOT NULL CHECK (method IN ('CASH', 'UPI', 'RAZORPAY', 'OTHER')),
