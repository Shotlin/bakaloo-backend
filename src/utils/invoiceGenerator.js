...
function drawTotals(doc, order) {
  ...
  // A wallet-partial-payment order (migration 112) or ledger-partial-
  // payment order (migration 127) has part of the total already deducted
  // from wallet/B2B credit at order-placement time, with the rest due via
  // whatever `payment_method` is (COD collects it on delivery, ONLINE
  // already captured it via Razorpay). Printing only the bare
  ...
  // way to know only ₹100 is actually still owed. The two never both
  // apply to the same order (see OrdersService#placeOrder), so only one
  // of walletUsed/ledgerUsed is ever nonzero.
  const walletUsed = parseFloat(order.wallet_amount_used || order.walletAmountUsed || 0)
  const ledgerUsed = parseFloat(order.ledger_amount_used || order.ledgerAmountUsed || 0)
  const methodLabel = paymentMethodLabel(order.payment_method || order.paymentMethod)

  doc.font('Helvetica').fontSize(8)
  if (walletUsed > 0 || ledgerUsed > 0) {
    const offsetLabel = ledgerUsed > 0 ? 'B2B Credit' : 'Wallet'
    const offsetAmount = ledgerUsed > 0 ? ledgerUsed : walletUsed
    ...
    const remaining = Math.max(0, total - offsetAmount)
    let y = doc.y
    doc.text(offsetLabel, PAGE_LEFT, y, { lineBreak: false })
    drawAmount(doc, offsetAmount, y, { size: 8 })
    doc.y = y + 13
