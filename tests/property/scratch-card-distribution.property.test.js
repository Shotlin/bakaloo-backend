// Scratch Card — weighted-pick distribution property tests.
// Mirrors spin-wheel-distribution.property.test.js exactly: pickWeightedPrize
// is duplicated (not shared) between the two gamification modules, so this
// suite duplicates its property coverage rather than assuming the sibling
// module's tests protect this one too.
//
// Properties covered:
...
import { pickWeightedPrize } from '../../src/modules/scratch-card/scratch-card.service.js'
...
describe('pickWeightedPrize (scratch-card) — boundaries', () => {
...
describe('pickWeightedPrize (scratch-card) — statistical distribution (100k trials)', () => {
  it('observed frequency stays within ±1.5 points of configured probability, for a skewed 5-prize card', () => {
    const prizes = [
...
describe('pickWeightedPrize (scratch-card) — property: distribution matches configured weights for arbitrary 2-8 prize cards', () => {
  const cardArb = fc
  ...
  it('every generated card sums to exactly 100 and is fully reachable', () => {
    fc.assert(
      fc.property(cardArb, (prizes) => {
